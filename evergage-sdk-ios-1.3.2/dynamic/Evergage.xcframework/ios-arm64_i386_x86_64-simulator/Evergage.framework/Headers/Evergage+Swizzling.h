...
/**
 ...
 Notification Service (APNS), simply ensure your UIApplicationDelegate defines the method
 `application:didRegisterForRemoteNotificationsWithDeviceToken:`.
 
 By default, Evergage will swizzle that method, if defined. However, if you
 disable swizzling in the Info.plist (see `<swizzlingEnabled>`), then you will
 need to manually call this method.
 
 This method notifies Evergage when the app's APNS token changes, in order to
 support Evergage push notification campaigns.
 
 This method will have no effect until you enable support for push notifications,
 ...
 Evergage automatically tracks clicks/opens from push notifications, but if you
 manually render custom UI on a foreground notification, you may want to use
 `<[EVGContext trackClickthrough:actionIdentifier:]>` to track clicks.
 
 Evergage can automatically determine if the token is for APNS production vs APNS sandbox.
...
/**
 You do not normally need to call this method. If your app uses Firebase Cloud
 ...
 This method notifies Evergage when the app's Firebase token changes, in order
 to support Evergage push notification campaigns.
 
 This method will have no effect until you enable support for push notifications,
 see `<[EVGClientConfigurationBuilder usePushNotifications]>`. When enabled,
 Evergage will also directly query Firebase Messaging (if present) for the token,
 but this method provides a timely update should a token change occur.
 
 Evergage will only send push notifications to the most recent user.
 See `<[Evergage userId]>` for details.
 
 Evergage automatically tracks clicks/opens from push notification, but if you
 manually render custom UI on a foreground notification, you may want to use
 `<[EVGContext trackClickthrough:actionIdentifier:]>` to track clicks.
