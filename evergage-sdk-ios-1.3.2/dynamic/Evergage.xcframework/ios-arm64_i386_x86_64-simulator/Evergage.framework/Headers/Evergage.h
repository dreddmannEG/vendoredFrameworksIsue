...
#if (TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR) && __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
#error "Evergage Framework requires iOS Deployment Target >= 9.0"
#endif
...
FOUNDATION_EXPORT NSString * _Nonnull const EvergageVersion; // 1.3.2
...
/**
 ...
 see `<[UIViewController(Evergage) evergageScreen]>` and `<EVGContext>` for the APIs.
 
 ## Startup
 Within `UIApplicationDelegate -willFinishLaunchingWithOptions:`, first enable logging
 in DEBUG builds. Set the `<userId>` as soon as the authenticated user ID is known.
 Finally, provide the app's Evergage configuration and start Evergage using
...
/**
 Typically unused, for tracking behavior outside of individual `UIViewController`s.
 
 Nil if Evergage is disabled.
 
 A typical UIViewController-based app should instead use the `<EVGScreen>` returned by
 `<[UIViewController(Evergage) evergageScreen]>`, which automatically cleans up resources and stops
 timing/tracking on UIViewController disappear.

 However, some apps have decoupled architectures/frameworks where referencing individual `UIViewController`s can be problematic/undesirable. In those scenarios, apps can choose to instead use this `globalContext`.
 ...
 - Stop accumulating view time on an item.
   - After calling any of the View APIs:
     - `<[EVGContext viewItem:]>`, `<[EVGContext viewItem:actionName:]>, `<[EVGContext viewItemDetail:]>`, `<[EVGContext viewItemDetail:actionName:]>`, `<[EVGContext viewCategory:]>`, `<[EVGContext viewCategory:actionName:]>`, `<[EVGContext viewTag:]>`, `<[EVGContext viewTag:actionName:]>`
